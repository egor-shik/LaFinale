...
func NextDate(now time.Time, dstart string, repeat string) (string, error) {
	...
	default:
		return "", errors.New("unsupported format of repeat")
	}
...
func nextWeek(now, start time.Time, parts []string) (string, error) {
	...
	for i := 1; i <= maxDaysSearch; i++ { //730 - примерно 2 года, для избежания бесконечного цикла
...
func nextMonth(now, start time.Time, parts []string) (string, error) {
	...
	for i := 0; i <= maxMonthsSearch; i++ { //ограничение в 2 года, как с днями
